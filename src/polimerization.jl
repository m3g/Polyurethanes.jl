...
# ╔═╡ 3e24c0bc-20d0-42f9-abaf-0216e1e395b2
using Plots

# ╔═╡ c4ca1f12-9d64-41df-ba0e-d25d9d03fc21
using Optim

# ╔═╡ ee6b02fc-bd2a-456d-9826-ed7572805c2b
begin
  using PlutoUI
  TableOfContents()
end

# ╔═╡ 0199e29e-30bb-40e5-9f72-84cc4552b1bb
md"# Definition of the reaction network"

# ╔═╡ 8eaf251a-2116-4c69-93b3-925586a35e58
md"Function that will be used to model the vaporization kinetics:"
...
# ╔═╡ 068cf273-2e24-46aa-b290-76e9f40eaf4c
md"Time span of the simulation (the same for all systems here):"

# ╔═╡ e6baeec5-0829-43fa-adc4-84d4cb5c61c2
tspan = (0.,350e3)

# ╔═╡ b2ef4c9a-232b-40de-af23-e330dc2839aa
md"# Model for parameter optimization"

# ╔═╡ 762ec4d0-91c1-485f-bb05-e4db61bdbe4e
md"## Score function for the parameter fit"

# ╔═╡ 86c56f4f-3006-41c5-a418-932aac260535
md"

$score = \sum_{i}^{N_{data}} \left[pred(i) - data(i)\right]^2$

Since the simulation and data points do not necessarily are obtained in the exact same times, the predicted values must be interpolated. 
"

# ╔═╡ e55bed02-2199-4f3c-9b86-aa27e5c5219a
function score(sim,data,sim_col)
  score = 0.
  p = getindex.(sim.u,sim_col)
  for (t,val) in eachrow(data)
...
# ╔═╡ 0fd7ca8f-8490-4def-b05f-04a3e5c47216
md"## Objective function for the optimization problem"

# ╔═╡ 9816ec28-1381-4a76-a65a-d398a03e244e
md"Evaluating the objective function corresponds to running a simulation and then computing the score of the fit:"

# ╔═╡ 9a6d7afa-9897-4af8-b5b1-156fdc747a44
function objective_function(p,ode_problem,experimental_data,sim_col)
	sim = solve(ode_problem,p=p)
	return score(sim,experimental_data,sim_col)
end

# ╔═╡ 9c5f9a34-5eb4-47a3-bedd-e782ce03f9cb
md"## Fitting function"

# ╔═╡ 3619b1c5-b1b3-4a07-ac5c-5c8fd81ee1c4
md"The fitting function will receive the initial guess for parameters, `p0`, the initial concentrations, `c0`, the time-span of the simulation `tspan`, the reaction network and the table of experimental data. The complete fitting procedure consists on running multiple simulations with different sets of parameters and keeping the best fit. This optimization will be performed with the Nelder-Mead optimization method, which is the default method of `Optim` when no derivatives are provided."

# ╔═╡ 77159304-509e-48f6-b4a2-9f8941c50142
...
# ╔═╡ 5c9cf397-417a-4a22-b567-ceaf2eea6a8b
md"# System: IPDI 0.25 100$^\circ$C"

# ╔═╡ c3a6c735-79b6-4727-b3a4-2ff9403bc998
md"## Experimental data"

# ╔═╡ 7807295f-defc-4d61-8c0b-8a1d0d08af5a
Exp_IPDI_025_100 = [ 
0	    6.68445
1800	3.34223
3600	2.33956
...
# ╔═╡ 59e8893c-2d0d-4af1-90f0-c3e776bf5121
md"## Sample simulation"

# ╔═╡ 3adc4e23-1629-4cf8-a222-fd3e4b671e71
md"Initial concentrations:"

# ╔═╡ 96b00550-e18c-4e21-9f1e-87d501b875e0
C0_IPDI_025_100 = (
	U = 5.01,
	NCO = 1.67,
	DIPA_l = 0.,	
...
# ╔═╡ cf52d15b-466e-4000-bbda-c6c2f0b79a03
md"Initial parameter guesses:"

# ╔═╡ 2b57906e-98c8-4b56-a70a-8a0851a04e94
P0_IPDI_025_100 = (
	k1 = 6.8e-6,
	km1 = 4.5e-4,
	k2 = 7.7e-5,
...
# ╔═╡ fd18f4aa-3e40-48be-a25b-2b097fa5bfcc
ode_IPDI_025_100 = ODEProblem(
	polimerization,
	[values(C0_IPDI_025_100)...],
	tspan,
	[values(P0_IPDI_025_100)...]
)

# ╔═╡ 589f5e61-ffc8-4ba1-a957-91022e6d6068
sim_IPDI_025_100 = solve(ode_IPDI_025_100)

# ╔═╡ bbb03113-d0a5-444e-b767-2a2822f25d03
md"Score of fit of the simulation to the data:"

# ╔═╡ df882073-0c0e-4da1-be08-05d4abc85813
score(sim_IPDI_025_100,Exp_IPDI_025_100,4)

# ╔═╡ 44ea4745-8316-4633-af5d-35bf6dfd9a61
md"## Optimization of the parameters"

# ╔═╡ 0c7a9978-8f4b-4c3e-ba5e-0a2e1f542d81
x = fit_parameters(
	P0_IPDI_025_100,
	C0_IPDI_025_100,
	tspan,
...
# ╔═╡ fe227caa-61e1-492e-9f4e-c82d7c897a15
md"# Notebook setup"

# ╔═╡ f0677352-c4db-44f8-a12a-a8bd88ffa40b
md"Packages required"

# ╔═╡ 1de09b46-74d0-4f49-848d-f95935d4f2ef
md"Generate table of contents"

# ╔═╡ 11d1f3f0-6834-4483-a9df-0749d1df25f7
md"Default plot style"

# ╔═╡ 8331ea84-ad00-462c-900b-23ed06c25b96
default(fontfamily="Computer Modern",
        linewidth=2, framestyle=:box, label=nothing, grid=false)

# ╔═╡ 6fec75eb-00bb-4b41-b72f-f169b0cc5ba3
scalefontsizes(1.2)

# ╔═╡ 36480bb2-9bb8-4968-8eeb-5f8cd59e95e4
function plot_OH(sim,data) 
	plt = plot()
	plot!(plt,
...
# ╔═╡ f0bfc39c-6b45-4e2a-80bf-28b4eafd7472
plot_OH(sim_IPDI_025_100,Exp_IPDI_025_100)
...
git-tree-sha1 = "dd981ebeecacc916d8506af723aea69096f6b08e"
